...
namespace Infraestructure.Core.Dapper
{
    public class DapperHelper
    {
        ...
        public int ExecuteQueryScalar(string query, object filter = null)
        {
            using (var conn = new SqlConnection(ConnectionString))
            {
                ...
                using (SqlTransaction tran = conn.BeginTransaction())
                {
                    try
                    {
                        var result = conn.ExecuteScalar<int>(query, filter, commandTimeout: bdTimeout, transaction: tran);
        ...
        /// <param name="filter">The filter.</param>
        /// <returns>System.Int32.</returns>
        public async Task<int> ExecuteQuery(string query, object filter = null)
        {
            using (var conn = new SqlConnection(ConnectionString))
            {
                conn.Open();
                using (SqlTransaction tran = conn.BeginTransaction())
                {
                    try
                    {
                        var result = await conn.ExecuteAsync(query, filter, commandTimeout: bdTimeout, transaction: tran, commandType: CommandType.StoredProcedure);
                        // if it was successful, commit the transaction
                        tran.Commit();
                        conn.Close();
                        conn.Dispose();
